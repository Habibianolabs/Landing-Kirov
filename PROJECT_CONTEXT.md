...
Короткая сводка готовности функций и оставшихся действий: [`PROJECT_STATUS.md`](PROJECT_STATUS.md).
...
[FACT] На 20 августа 2026 года локальная папка и GitHub `main` синхронизированы. `g10-kirov.vercel.app` используется только для демонстрации frontend; это не подтверждение работы PHP и почты на IHC.
...
[FACT] Прямая загрузка JavaScript-файла не запускает серверный код на IHC. Для публичного домена добавлен отдельный PHP-маршрут `/api/submit-application.php`; 20 августа `OPTIONS` подтвердил его исполнение PHP 7.3.33. Новый hotfix rate limit ожидает загрузки на IHC.
...
[FACT] Production-приём заявок частично активирован: PHP-файл исполняется на IHC и отклоняет неполную заявку ответом `400`. Валидная техническая заявка получила `503` до почтовой функции из-за недоступного файлового rate limit. Получатели и адрес отправителя зафиксированы в серверном файле; hotfix должен быть загружен перед повторной проверкой письма.

Аналитика загружается только после analytics consent. [FACT] 18 августа 2026 года создан счётчик Яндекс Метрики `G10 Kirov` № `111727875`; код опубликован на IHC в предыдущей синхронизации. [FACT] 20 августа получен код EnvyBox и добавлен consent-aware loader в `script.js`; проверка на домене заказчика подтвердила загрузку CSS, JavaScript и API, но сервис вернул «Виджеты не включены». Активация трёх модулей остаётся задачей заказчика. Vercel Analytics удалена из статической версии из-за ответа `404`.
...
- загрузка hotfix PHP-обработчика и подтверждение PHP/mail на IHC;

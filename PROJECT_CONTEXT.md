...
Аналитика загружается только после analytics consent. [FACT] 18 августа 2026 года создан счётчик Яндекс Метрики `G10 Kirov` № `111727875`; код готов в `script.js`, но ещё не опубликован на отдельной IHC-копии. [FACT] 20 августа получен код EnvyBox и добавлен consent-aware loader в `script.js`; локальная проверка загрузчика прошла, но настройки сервиса вернули «Виджеты не включены». Активация трёх модулей и публикация актуальных runtime-файлов на IHC остаются открытыми.

...
[FACT] Локальная папка и открытый GitHub `main` совпадают; актуальная версия опубликована в Vercel production. Перед публикацией на IHC нужно отдельно сравнить runtime-файлы; папка `api/` не запускает Vercel Function на обычном статическом сервере автоматически.

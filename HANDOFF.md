...
[FACT] Локальная папка и открытый GitHub `main` совпадают на коммите `7d574f3`; актуальная версия опубликована в Vercel deployment `dpl_2azSzQvH43xdZ92xRJCRc6MpfT1N`. Перед публикацией на IHC нужно отдельно сравнить runtime-файлы; папка `api/` не запускает Vercel Function на обычном статическом сервере автоматически.
...
Последние проверки: `node --check script.js`, `node --check api/submit-application.js`, `git diff --check`, desktop/mobile smoke test списка экспертов, проверка отсутствия горизонтального переполнения и сверка контрольных сумм с GitHub/Vercel. Последняя синхронизированная версия GitHub — `7d574f3`; IHC-origin пока не синхронизирован. Не включать `.env.local` или `.vercel/.env.production.local` в документы, Git или архив; если локальный токен когда-либо раскрыт, его нужно отозвать и выпустить заново.
